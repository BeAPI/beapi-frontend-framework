...
class WebpackImageSizesPlugin {
  ...
  constructor(options = {}) {
    ...
    // Track last generation timestamp to avoid unnecessary rebuilds
  ...
  apply(compiler) {
    ...
    const runGeneration = async (compilation, callback) => {
      try {
        ...
        // Check if we need to regenerate based on file changes
        if (this.shouldSkipGeneration(confImgPath, sizesPath, tplPath)) {
          this.log('log', '⏭️  No changes detected, skipping generation')
          if (callback) {
            callback()
          }
          return
