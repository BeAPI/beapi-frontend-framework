...
class BEA_Images{
	...
	public static function load_image_sizes(){
		...
		$json_content = file_get_contents( BEA_IMAGES_JSON_DIR.'image-sizes.json' );
		$result = json_decode( $json_content );
	...
	public static function load_locations(  ){
		...
		$json_content = file_get_contents( BEA_IMAGES_JSON_DIR.'image-locations.json' );
		$result = json_decode( $json_content );
		if( !is_array( $result ) || empty( $result ) ){
			return false;
		}
	...
	public static function load_hooks(  ){
		...
		$json_content = file_get_contents( BEA_IMAGES_JSON_DIR.'image-hooks.json' );
		$result = json_decode( $json_content );
		if( !is_array( $result ) || empty( $result ) ){
			return false;
		}
